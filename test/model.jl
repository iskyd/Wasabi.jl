@testset "model" begin
    ...
    @test Wasabi.model2tuple(u) == ((:id, nothing), (:name, "John"), (:created_at, u.created_at))

    df = DataFrame(id = [1, 2, 3], name = ["John", "Jane", "Joe"], created_at = [now(), now(), now()])
    models = Wasabi.df2model(User, df)

    @test models[1].id == Wasabi.AutoIncrement(1)
    @test models[2].id == Wasabi.AutoIncrement(2)
    @test models[3].id == Wasabi.AutoIncrement(3)
