@testset "query builder" begin
    ...
    sql, params = QueryBuilder.build(query)
    @test sql.value == "SELECT user_alias.id, user_alias.name, user_alias.created_at, user_profile_alias.bio FROM \"user\" user_alias INNER JOIN \"user_profile\" user_profile_alias ON user_alias.id = user_profile_alias.user_id WHERE (user_alias.id IN \$1 OR user_profile_alias.bio = \$2)"
